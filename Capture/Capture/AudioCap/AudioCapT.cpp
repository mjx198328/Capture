...
UINT __stdcall CAudioCapT::_CaptureThreadProc(LPVOID param)
{
	...
	do
	{
		...
		if (nDeviceType == CAudioCapT::MICPHONE)
			hr = pAudioClient->Initialize(AUDCLNT_SHAREMODE_SHARED, 0, REFTIMES_PER_SEC, 0, pWfx, 0);
		else if (nDeviceType == CAudioCapT::SPEAKER)
			hr = pAudioClient->Initialize(AUDCLNT_SHAREMODE_SHARED, AUDCLNT_STREAMFLAGS_LOOPBACK, REFTIMES_PER_SEC, 0, pWfx, 0);
		else
			break;
		...
		while (pObject->m_bStop == false)
		{
			...
			if (uiNextPacketSize == 0)
			{
				//Ð´Èë¾²Ä¬Òô

				BYTE * pBuf = new BYTE[bufferFrameCount / 2 * pWfx->nBlockAlign];
				if (pBuf)
				{
					memset(pBuf, 0, bufferFrameCount / 2 * pWfx->nBlockAlign);
					pObject->OnCaptureData(pBuf, bufferFrameCount / 2 * pWfx->nBlockAlign);
					delete[] pBuf;
				}
			}
			else
			{
				while (uiNextPacketSize != 0)
				{
					...
					{
						memset(pData, 0, uiNumFramesToRead * pWfx->nBlockAlign);
					}
